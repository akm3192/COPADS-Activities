            ...
            if (message.StartsWith("/"))
            {
                ...
                switch (command)
                {
                    case "time":
                        ...
                        writer.WriteLineAsync($"[ {time} ]");
                        break;
                    case "date":
                        var now = DateTime.Now.ToString("yyyy-MM-dd");
                        writer.WriteLineAsync(now);
                        break;
                    case "help":
                        await writer.WriteLineAsync("\nAvailable Commands:");
                        await writer.WriteLineAsync("  /time  - current server time (HH:mm:ss format)");
                        await writer.WriteLineAsync("  /date  - current date (yyyy-MM-dd format)");
                        await writer.WriteLineAsync("  /help  - see list of available commands");
                        await writer.WriteLineAsync("  /quit  - Exit the application");
                        await writer.WriteLineAsync();
                        break;
                    case "quit":
                        writer.WriteLineAsync("Goodbye!");
                        tcpClient.Client.Disconnect(true);
                        break;
                    default:
                        writer.WriteLineAsync("Unknown command.");
                        break;
                }
...
async Task TestClient(int serverPort)
{
    try
    {
        ...
        // Test messages
        string[] messages = { "Hello, Server!","/date", "/time", "/help"};
